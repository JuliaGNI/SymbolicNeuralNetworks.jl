...
function apply_build_function(input_dim::Integer=2, output_dim::Integer=1, num_examples::Integer=3)
    ...
    eq = (c(snn.input, snn.params) - soutput).^2
    built_function = _build_nn_function(eq, snn.params, snn.input, soutput)
    input = rand(input_dim, num_examples)
    output = rand(output_dim, num_examples)

    @test all(i -> (built_function(input, output, nn.params, i) ≈ (c(input[:, i], nn.params) - output[:, i]).^2), 1:num_examples)
end
