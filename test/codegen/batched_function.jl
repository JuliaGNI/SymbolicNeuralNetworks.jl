...
@testset "three-dimensional input, reduce = $reduction, inplace = $inplace" for
        reduction in (hcat, +), inplace in (true, false)
    ...
    result = f(input, ps)
    if reduction === hcat
        @test size(result) == (OUTPUT_DIM, 2, 3)
        for i in 1:2, j in 1:3
            @test result[:, i, j] ≈ c(input[:, i, j], ps)
        end
    else
        # the samples were summed, so there is no batch dimension left to restore
        @test result ≈ reference((x, p) -> c(x, p), ps, flat, +)
    end
end
